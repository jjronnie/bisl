<x-slide-form buttonIcon="eye" title="Transaction ID: {{ $transaction->reference_number }}">
      ...
      <div class="bg-white p-6 ">
            <dl class="grid grid-cols-2 md:grid-cols-4 gap-6">
                  ...
                  <x-transaction-detail title="Balance Before"
                        value="{{ number_format($transaction->balance_before, 2) }}" />

                  <x-transaction-detail title="Balance After"
                        value="{{ number_format($transaction->balance_after, 2) }}" />
