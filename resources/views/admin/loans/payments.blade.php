...
<div class="bg-white p-6 rounded-lg shadow-md mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-6">
        <p><strong>Member:</strong> {{ $loan->member->first_name }} {{ $loan->member->last_name }} ({{ $loan->member->email }})</p>
        ...
        @if($installment)
            ...
            <p class="text-lg font-bold">
                <strong>Amount Due (Incl. Penalty):</strong> 
    ...
    @if($installment)
        <form method="POST" action="{{ route('admin.payments.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <p class="mt-2 text-sm text-gray-500">Enter the exact amount received from the member.</p>
            ...
            <div class="flex justify-end space-x-3 pt-4">
                <a href="{{ route('admin.loans.show', $loan->id) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-lg shadow">
                    Cancel
                </a>
                <button type="submit" class="bg-blue-600 text-white hover:bg-blue-700 font-bold py-2 px-4 rounded-lg shadow-lg">
                    Confirm Payment
                </button>
            </div>
