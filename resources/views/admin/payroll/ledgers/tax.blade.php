<x-app-layout>
    <x-page-title title="Tax Ledger" subtitle="Aggregated PAYE tax payable account" />

    <div x-data="{ loaded: true, showWithdraw: false }" x-show="loaded" x-transition.duration.500ms class="space-y-6">

        {{-- Account Summary --}}
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <x-stat-card title="Current Balance" value="UGX {{ number_format($account->balance, 0) }}" icon="landmark" sub_title="Total PAYE collected" />
            ...
            <x-stat-card title="Total Withdrawn" value="UGX {{ number_format($account->total_withdrawn, 0) }}" icon="arrow-down" sub_title="Paid to URA" />
        </div>

        {{-- Withdraw Form --}}
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-gray-800">Withdraw from Tax Account</h2>
            ...
            <div x-show="showWithdraw" x-collapse.duration.300ms>
                <form method="POST" action="{{ route('admin.payroll.ledgers.withdraw', $account) }}" class="max-w-lg space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="reason" rows="2" class="border-gray-300 rounded-md shadow-sm w-full text-sm" placeholder="e.g. Monthly PAYE remittance to URA" required></textarea>
                    </div>
