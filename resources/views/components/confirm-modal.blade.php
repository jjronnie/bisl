...
<div x-data="{ showModal: false, acknowledged: false }" class="inline-block">
    ...
    <div x-show="showModal" x-transition
        ...
        <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6" @click.away="showModal = false">
            ...
            <p class="mt-2 text-sm text-yellow-600 break-words whitespace-normal">{{ $warning }}</p>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                ...
                <form method="POST" action="{{ $action }}">
                    ...
                    <button type="submit" :disabled="!acknowledged"
                        class="px-4 py-2 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed {{ $buttonClass }}">
                        {{ $buttonText }}
                    </button>
                </form>
            </div>
